  ...
  <?php foreach($parent as $p):?>
    ...
    <?php else:?>
      <li class="al-sidebar-list-item">
        <?php $title = '';?>
        <?php if ($p['name']=='SPPKD'):?>
        <?php $title = 'Surat Penunjukan Penggunaan Kendaraan Dinas';?>
        <?php endif;?>
        <a class="al-sidebar-list-link" href="<?=Url::to([$p['route']]);?>" data-title="<?=$title;?>">
          <i class="<?=$p['icon'];?>"></i>
          <span><?=$p['name'];?></span>
        </a>
      </li>
...
<div class="page-top clearfix">
    <a href="#/dashboard" class="al-logo clearfix">
        ...
        <img src="<?=Yii::getAlias('@web');?>/img/logo_kendi.png" style="width:120px;height:60px;display:inline" class="img-responsive"/>
        </div>
    </a> 
    <a href="#" class="collapse-menu-link ion-navicon"></a>
    <div class="user-profile clearfix">
      <div class="al-user-profile dropdown" uib-dropdown="">
        <a class="profile-toggle-link dropdown-toggle" aria-haspopup="true" aria-expanded="false"><i class="fa fa-user fa-3x"></i></a>
        <ul class="top-dropdown-menu profile-dropdown dropdown-menu" uib-dropdown-menu=""><li><i class="dropdown-arr"></i></li>
          <li><a href="<?=Url::to(['/site/profile']);?>"><i class="fa fa-user"></i>Profile</a></li>
          <li><a href="<?=Url::to(['/site/logout']);?>" data-confirm="keluar Dari Aplikasi?" data-method="POST" class="signout"><i class="fa fa-power-off"></i>Sign out</a></li></ul></div></div>
</div>
